...
decode_sub_msg(nano_work *work)
{
	...
	if (MQTT_PROTOCOL_VERSION_v5 == proto_ver) {
		sub_pkt->properties = decode_properties(
		    msg, (uint32_t *) &vpos, &sub_pkt->prop_len, true);
	...
	while (1) {
		...
		log_info(
		    "topic: [%s] len: [%d]", tn->topic.body, len_of_topic);
		len_of_topic = 0;
		...
		if (MQTT_VERSION_V5 == proto_ver &&
		    strncmp(tn->topic.body, "$share/", strlen("$share/")) ==
		        0 &&
		    tn->no_local == 1) {
			tn->reason_code = UNSPECIFIED_ERROR;
			return PROTOCOL_ERROR;
...
encode_suback_msg(nng_msg *msg, nano_work *work)
{
	...
	if (MQTT_PROTOCOL_VERSION_v5 ==
	    proto_ver) { // add property in variable
	...
	// Note. When packet_id is zero, topic node must be empty. So, Dont
	// worry about that the order of reason codes would be changed. handle
	// payload
	tn = sub_pkt->node;
	while (tn) {
...
sub_ctx_handle(nano_work *work)
{
	...
	while (tn) {
		...
		work->msg_ret =
		    (work->msg_ret == NULL) ? retain : work->msg_ret;
